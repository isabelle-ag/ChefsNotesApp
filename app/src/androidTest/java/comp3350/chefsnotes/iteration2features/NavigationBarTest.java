...
public class NavigationBarTest {
    ...
    @Test
    public void testCreateRecipeButton()
    {
    ...
    public void testBrowseButton()
    {
        ...
        onView(withId(R.id.searchRecipeName)).check(matches(withText("#3350TEST")));
    }

    @Test
    public void testCurrentRecipeButton()
    {
        onView(withId(R.id.browse_recipe_button)).perform(click());
        onView(withId(R.id.searchRecipeName)).perform(typeText("#3350TEST"));
        onData(anything()).atPosition(0).perform(click());
        onView(withId(R.id.recipeName)).check(matches(allOf(isDisplayed(), withText(example.getTitle()))));
        onView(withId(R.id.browse_recipe_button)).perform(click());
        onView(withId(R.id.current_recipe_button)).perform(click());
        onView(withId(R.id.recipeName)).check(matches(allOf(isDisplayed(), withText(example.getTitle()))));
